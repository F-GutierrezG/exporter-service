...
ssh -o StrictHostKeyChecking=no ubuntu@${PRODUCTION_SERVER} "docker run --log-driver=awslogs --log-opt awslogs-region=us-east-2 --log-opt awslogs-group=ExporterServiceSwagger -d --restart always -e 'API_URL=definitions/swagger.yml' --name exporter-swagger --network exporter-service-network --ip 172.24.0.3 $REGISTRY_REPO/$SWAGGER:$TAG"
ssh -o StrictHostKeyChecking=no ubuntu@${PRODUCTION_SERVER} "docker run --log-driver=awslogs --log-opt awslogs-region=us-east-2 --log-opt awslogs-group=ExporterService -d --restart always -e 'FLASK_ENV=production' -e 'FLASK_APP=manage.py' -e 'APP_SETTINGS=project.config.ProductionConfig' -e 'SECRET_KEY=$PROD_SECRET_KEY' -e USERS_SERVICE_URL=$USERS_SERVICE_URL -e AUTH_SERVICE_URL=$AUTH_SERVICE_URL -e COMPANIES_SERVICE_URL=$COMPANIES_SERVICE_URL -e SOCIAL_SERVICE_URL=$SOCIAL_SERVICE_URL --name exporter --network exporter-service-network --ip 172.24.0.2 $REGISTRY_REPO/$EXPORTER:$TAG"

ssh -o StrictHostKeyChecking=no ubuntu@${PRODUCTION_SERVER} 'docker network connect onelike-network --ip 172.18.0.14 exporter'
ssh -o StrictHostKeyChecking=no ubuntu@${PRODUCTION_SERVER} 'docker network connect onelike-network --ip 172.18.0.15 exporter-swagger'
